...
import grails.plugin.blazeds.MySpringPropertyProxy
...
'''
    ...
    private initDomainClassProxies = {ctx ->
        ...
        for (classMetadata in ctx.sessionFactory.allClassMetadata.values()) {
            ...
            MySpringPropertyProxy proxy = MySpringPropertyProxy.proxyFor(classMetadata.getMappedClass(EntityMode.POJO), false, conversionService)
            proxy.setIgnoreNames(ignoreNames)
...
//	private void initDomainClassProxies(ctx) {
    ...
    //		// register a proxy for all domain classes to limit auto-serialization of metaclass, etc.
    //		for (classMetadata in ctx.sessionFactory.allClassMetadata.values()) {
    //            def proxy = BlazeDsPropertyProxy.proxyFor(classMetadata.getMappedClass(EntityMode.POJO), false, conversionService);
    //            proxy.setIgnoreNames(ignoreNames)
    //			debug "created domain class proxy for ${classMetadata.getMappedClass(EntityMode.POJO)}"
